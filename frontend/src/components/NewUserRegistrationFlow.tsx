...
export function NewUserRegistrationFlow({ onSuccess, onBack }: NewUserRegistrationFlowProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const apiDesc =
        typeof resMsg !== 'undefined' ? resMsg : SIGNUP_PENDING_DESCRIPTION;
      ...
      <CardContent>
        <Alert className="mb-5 border-primary/30 bg-primary/8">
          ...
          <AlertDescription className="text-foreground text-sm">
            <strong>Quick sign-up:</strong> Register with your details. Verification with Ghana Lands Commission and NIA (Ghana Card) is{' '}
            <strong>not immediate</strong> — expect an email within <strong>{VERIFICATION_HOURS_RANGE}</strong>. Complete Ghana Card steps after login.
          </AlertDescription>
        </Alert>

...
export function ParcelChatDialog({ open, onOpenChange, landParcelId, parcelTitle }: ParcelChatDialogProps) {
  ...
  const landsCommissionPhone = (import.meta.env.VITE_LANDS_COMMISSION_PHONE as string | undefined) ?? '';
  const arbitratorPhone = (import.meta.env.VITE_ARBITRATOR_PHONE as string | undefined) ?? '';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = Array.isArray(parcels) ? parcels : (parcels?.parcels ?? parcels?.data ?? []);
        const mapped = (Array.isArray(list) ? list : [])
          .filter((p: any) => {
            // Try to support both backend shapes
            const sellerId = p?.sellerId || p?.seller?.id || p?.ownerId || p?.owner?.id;
            const status = String(p?.status || '').toLowerCase();
            return sellerId === otherParty.id && (status === 'available' || status === ''); // default to show
          })
          .map((p: any) => ({
            id: String(p.id),
            title: String(p.title || p.land_name || p.name || 'Parcel'),
            priceGhs: typeof p.priceGhs === 'number' ? p.priceGhs : typeof p.price === 'number' ? p.price : undefined,
            status: p.status,
          }));

...
function NiaBadge({ idVerification, niaStatus, niaReferenceId }: { idVerification: string | null | undefined; niaStatus?: string | null; niaReferenceId?: string | null }) {
  const parsed = parseIdVerification(idVerification);
  if (niaStatus === 'verified') {
    return (
      <Badge className="bg-primary text-primary-foreground gap-1.5 px-2.5 py-1">
        <ShieldCheck className="w-3.5 h-3.5" />
        NIA: Verified ✓ {niaReferenceId ? <span className="font-mono text-[10px] opacity-90">{niaReferenceId}</span> : null}
      </Badge>
    );
  }
  if (niaStatus === 'rejected') {
    return (
...
export function RegistrationReview() {
  ...
  const canApprove = (u: PendingUser) => u.niaStatus === 'verified';

  const orderedUsers = [...pendingUsers].sort((a, b) => {
    const aReady = a.niaStatus === 'verified' ? 2 : a.idVerification ? 1 : 0;
    const bReady = b.niaStatus === 'verified' ? 2 : b.idVerification ? 1 : 0;
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4 p-5 pb-3">
                ...
                <NiaBadge idVerification={u.idVerification} niaStatus={u.niaStatus} niaReferenceId={u.niaReferenceId} />
              </div>
                  ...
                  {!parsedCard ? (
                    <div className="rounded-lg border border-amber-200 bg-amber-50/60 p-3">
                      ...
                      <p className="text-xs text-amber-800 mt-0.5">
                        The user has not completed their Ghana Card upload yet. Ask them to submit Ghana Card details for NIA verification.
                      </p>
                    </div>
                  ) : (
              ...
              <div className="flex items-center justify-between gap-3 px-5 py-4 flex-wrap">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (!canApprove(u)) {
                        toast.error('NIA verification required', {
                          description: 'This user must be verified in the NIA dashboard before Lands Commission can grant access.',
                          duration: 7000,
                        });
                        return;
                      }

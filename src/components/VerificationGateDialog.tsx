...
export function VerificationGateDialog() {
  ...
  const handleVerificationComplete = (data: VerificationData) => {
    const verificationPayload = { ...data, status: 'verified' as const };
    updateUser({ verificationStatus: 'verified', idVerification: verificationPayload });
    // Keep the admin's pending users store in sync with the Ghana Card data
    if (user?.id) updateLocalPendingUserVerification(user.id, verificationPayload);
    setDismissed(true);
    toast.success('Identity verified! You now have full access to all features.');
  };
            ...
            <div className="px-6 py-5 space-y-5">
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
                ...
                <p className="text-sm text-amber-800 dark:text-amber-300 leading-relaxed">
                  Ghana Land Registry requires identity verification to protect all parties in land transactions. This is a legal requirement under the Land Act 2020.
                </p>
              </div>
